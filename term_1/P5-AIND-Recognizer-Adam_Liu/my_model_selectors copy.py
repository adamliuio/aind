	...
	def select(self):
		...
		for components_num in range(self.min_n_components, self.max_n_components):
			...
			try:
				...
				# # logging the likelihood of the model
				# logL  = model.score(self.X, self.lengths)
				# features_num = self.X.shape[1]    # previous
				# # number of parameters
				# params_num = components_num * (components_num - 1) + 2 * features_num * components_num + components_num - 1    # previous
				# # calculating BIC
				# bic_score = -2 * logL + params_num * np.log(features_num)    # previous
